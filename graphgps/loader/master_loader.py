...
# mjn - need random for ZINC-FT
import random
# mnj - also need heapq for ZINC-FT
import heapq
# mjn - for modifying y-values of CIFAR10
from torch_geometric.data import Data
# mjn - for flipping the y-value of cifar10-FT via 
from graphgps.transform.transforms import flip_y, remove_edges, mask_features
...
def load_dataset_master(format, name, dataset_dir):
    ...
    if format.startswith('PyG-'):
        ...
        # mjn: to get the cifar10 fine-tuning dataset
        elif (pyg_dataset_id == 'GNNBenchmarkDataset-FT' or pyg_dataset_id == 'GNNBenchmarkDataset-FT1') and name in ['CIFAR10', 'CLUSTER']:
            dataset = preformat_FT_GNNBenchmarkDataset(dataset_dir, name)
            #print(dataset[0].y.item())
            #for data in dataset:
            #    new_y = torch.tensor([9 - data.y.item()])
            #    data.y = new_y
            #print(dataset[0].y.item())
        ...
        # mjn: to get the ZINC fine-tuning dataset
        elif pyg_dataset_id == 'ZINC-FT':
            dataset = preformat_FT_ZINC(dataset_dir, name)

        # mjn: to get the ZINC fine-tuning dataset
        elif pyg_dataset_id == 'ZINC-FT1':
            dataset = preformat_FT1_ZINC(dataset_dir, name)
...
def get_subset(dataset, num_samples=1000):
    # Assumes the dataset is large enough; otherwise, adjust `num_samples` as needed.
    indices = list(range(num_samples))
    return dataset.index_select(indices)

def preformat_FT_GNNBenchmarkDataset(dataset_dir, name):
    ...
    # mjn - transform the data for the fine-tining task (flip y-values for CIFAR10-FT, ML or mask for CLUSTER): 
    data_trans_func = None
    num_graphs = 1000
    if name == 'CIFAR10':
        data_trans_func = flip_y(9)
    if name == 'CLUSTER':
        num_graphs = 600
        if cfg.dataset.masking_or_ML == "masking":
            print("use masking data transform")
            mask = [1,3,6]
            data_trans_func = mask_features(mask)
        else:
            print("use missing links data transform")
            data_trans_func = remove_edges()
    if data_trans_func == None:
        raise Exception("Something seems to be wrong in the config file you use, or with this code.")
    tf_list.append(data_trans_func)

    if name in ['MNIST', 'CIFAR10', 'PATTERN', 'CLUSTER']:
        dataset = join_dataset_splits(
            [get_subset(GNNBenchmarkDataset(root=dataset_dir, name=name, split=split), num_samples=num_graphs)
            for split in ['train', 'val', 'test']]
        )
        # mjn - print y-value to compare with result after transforming
        print(dataset[0].x[1])
...
def preformat_GNNBenchmarkDataset(dataset_dir, name):
    ...
    Returns:
        PyG dataset object
    """
    if name in ['MNIST', 'CIFAR10', 'CIFAR10-FT', 'CIFAR10-FT1']:
        tf_list = [concat_x_and_pos]  # concat pixel value and pos. coordinate
        tf_list.append(partial(typecast_x, type_str='float'))
    elif name in ['PATTERN', 'CLUSTER', 'CSL']:
        tf_list = []
...
def get_minus_y(obj):
    return (-1) * float(obj.y[0])

def preformat_FT1_ZINC(dataset_dir, name):
    # mjn - flip the y-values for CIFAR10-FT and CLUSTER
    tf_list = []
    edge_remove_func = remove_edges()
    tf_list.append(edge_remove_func)

    dataset = join_dataset_splits([get_subset(ZINC(root=dataset_dir, subset=False, split=split), 600)
         for split in ['train', 'val', 'test']])
...
def preformat_FT_ZINC(dataset_dir, name):
    ...
    Returns:
        PyG dataset object
    """
    full_zinc_datasets = [ZINC(root=dataset_dir, subset=False, split=split)
         for split in ['train', 'val', 'test']]
    
    full_index_tuples = [[(i, get_minus_y(dataset[i])) for i in range(len(dataset))]
        for dataset in full_zinc_datasets]

    train_size, val_size, test_size = 600, 200, 200
    highest_tuples = [
        heapq.nsmallest(train_size, full_index_tuples[0], key=lambda t : t[1]),
        heapq.nsmallest(val_size, full_index_tuples[1], key=lambda t : t[1]),
        heapq.nsmallest(test_size, full_index_tuples[2], key=lambda t : t[1])
    ]

    #print(highest_tuples)

    FT_indices = [[tuple[0] for tuple in tuples]
        for tuples in highest_tuples]

    #print(FT_indices)

    random_instance = random.Random(2802)
    for indices in FT_indices:
        random_instance.shuffle(indices)

    FT_zinc_datasets = [full_zinc_datasets[i].index_select(FT_indices[i])
        for i in range(3)]

    dataset = join_dataset_splits(FT_zinc_datasets)
    return dataset
